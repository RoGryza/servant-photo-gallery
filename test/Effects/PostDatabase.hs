...
genMedia = MediaF <$> Gen.element Corpus.glass
                  <*> Gen.enumBounded
...
stFetch t l (State xs) = take (fromIntegral l) . filter ((< t) . tpCreatedAt) . sortOn (Down . tpCreatedAt) $ xs
...
cFetchPosts = let
  gen (State s) = Just $ do
    ts <- if null s then genTimestamp else
            Gen.frequency [ (1, Gen.element $ tpCreatedAt <$> s)
                          , (4, genTimestamp)
                          ]
  ...
  in
    Command gen execute
      [ Ensure $ \_ s@(State inner) (FetchPosts t l) o -> do
          let ltPosts = filter ((< t) . tpCreatedAt) inner
...
cInsertMedia = let
  gen (State inner) =
    let 
      withoutMedia = filter (isNothing . tpMedia) inner
    in
...
prop_db_state_equivalent conn = withTests 100 . property . hoistTest conn $ do
  actions <- forAll $
    ...
    (Range.linear 1 100)
    initialState
    [cFetchPosts, cInsertPost, cInsertMedia]
